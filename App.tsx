...
export default function App() {
        ...
        for (const line of lines) {
          if (line.startsWith('data: ')) {
            ...
            if (data.download_id) {
              ...
              setIsProcessing(false); // Detener el cargando SOLO AQUÍ
            }
          }
        }
      }
    } catch (err) {
      setError("Error fatal en el motor. Verifica tu conexión.");
  ...
  return (
    <div className="min-h-screen bg-black text-white p-6 flex flex-col items-center justify-center font-sans">
      <div className="w-full max-w-xl bg-zinc-950 border border-zinc-900 rounded-[2.5rem] p-10 shadow-2xl relative">
        ...
        <div className="space-y-6">
          {!isProcessing && !downloadId && (
            <label className="border-2 border-dashed border-zinc-800 hover:border-red-600 rounded-3xl p-16 flex flex-col items-center justify-center cursor-pointer bg-zinc-900/10">
          ...
          {file && !isProcessing && !downloadId && (
            <button onClick={processAudio} className="w-full py-5 bg-red-600 hover:bg-red-700 text-white font-black rounded-2xl">INICIAR LIMPIEZA PROFESIONAL</button>
          )}
          ...
          {downloadId && (
            <div className="p-8 bg-zinc-900/30 border border-green-900/20 rounded-3xl text-center space-y-6">
              ...
              <a 
                href={`${apiUrl.replace(/\/$/, '')}/download/${downloadId}`} 
                className="flex items-center justify-center gap-3 w-full py-5 bg-white text-black font-black rounded-2xl shadow-xl"
              >
                <Download size={20} /> DESCARGAR AUDIO LIMPIO
              </a>
              <button onClick={() => {setFile(null); setDownloadId(null);}} className="text-zinc-500 text-xs underline">Procesar otro</button>
            </div>
          )}
      ...
      {showSettings && (
        <div className="fixed inset-0 bg-black/95 flex items-center justify-center z-50 p-6">
          <div className="bg-zinc-950 border border-zinc-900 p-8 rounded-3xl w-full max-w-sm">
            ...
            <input type="text" value={apiUrl} onChange={(e) => setApiUrl(e.target.value)} className="w-full bg-black border border-zinc-800 p-4 text-red-500 mb-6" />
